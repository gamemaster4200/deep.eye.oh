...
def test_robots_txt_unreachable_records_failure_and_exits_cleanly(tmp_path, monkeypatch):
    """robots.txt being unreachable (e.g. blocked for this client) must
    refuse to proceed and record the attempt -- never silently skip the
    robots.txt check and never raise an unhandled traceback."""

    def fake_build_robot_parser(base_url, *, user_agent, timeout, opener=None):
        raise _http.PermanentAcquisitionError("permanent HTTP error 403 for robots.txt", http_status=403)

    monkeypatch.setattr(discover._http, "build_robot_parser", fake_build_robot_parser)

    out_path = tmp_path / "pages_index.json"
    with pytest.raises(SystemExit):
        discover._build_robot_parser_or_record_failure("https://example.fandom.com", user_agent="ua/1", timeout=5.0, out_path=out_path)
...
def test_build_pages_index_shape():
    ...
    index = discover.build_pages_index("https://example.fandom.com", namespaces, pages)
